...
sfr_cols = [keep_idx.index(i) for i, n in enumerate(param_names)
            if n.startswith("logsfr_ratio")]

cfg = dict(
    arch="resmlp", width=run_cfg["width"], depth=run_cfg["depth"],
    blocks=run_cfg["blocks"], mass_norm=True, keep_mass_input=False,
    sfr_arsinh=True, fourier_k=run_cfg["fourier_k"],
    flux_floor=lab.FLUX_FLOOR, mu0_train=float(mu0_train),
    i_mass=i_mass, i_z=i_z, keep_idx=keep_idx, sfr_cols=sfr_cols,
)

emu = ParrotEmulatorV2(param_names, band_names, cfg, in_mean, in_std, out_mean,
...
for i in range(0, len(te), 65536):
    pred.append(np.asarray(emu2.predict(jnp.array(params_raw[te[i:i + 65536]],
                                                  dtype=jnp.float32))))
