...
LIBRARY = ("/cosma/apps/dp276/dc-harv3/synference/libraries/"
           "library_BPASS_Chab_Continuity_SFH_0.01_z_14_logN_6.0_Calzetti_IGM_Asada25_v6_multinode.hdf5")
REF_CHECKPOINT = ("/cosma/apps/dp276/dc-harv3/arachne/scripts/outputs/emulators/"
                  "parrot_emulator.eqx")
...
def parse_args(argv=None):
    ...
    p.add_argument("--fourier-k", type=int, default=0,
...
class FeatureSpec:
    ...
    def __init__(self, param_names, mass_norm, sfr_arsinh, fourier_k,
        ...
        self.keep = [i for i in range(len(param_names))
    ...
    if arch == "mlp":
        class MLP(eqx.Module):
            ...
            def __init__(self, key):
                ...
                self.layers = tuple(eqx.nn.Linear(a, b, key=k)
                                    for a, b, k in zip(sizes[:-1], sizes[1:], ks))

            def __call__(self, x):
                for l in self.layers[:-1]:
                    x = jax.nn.gelu(l(x), approximate=True)
...
def main(argv=None):
    ...
    if args.eval_checkpoint:
        ...
        for i in range(0, len(te), 65536):
            pred.append(np.asarray(emu.predict(jnp.array(params_raw[te[i:i + 65536]],
                                                         dtype=jnp.float32))))
        pred_flux = np.concatenate(pred).astype(np.float64)
        report(args, band_names, params_raw[te], flux_test_true, mag_test_true,
               pred_flux, n_params=None, train_s=0.0)
        return 0
    ...
    fs = FeatureSpec(param_names, args.mass_norm, args.sfr_arsinh, args.fourier_k,
    ...
    sched = optax.warmup_cosine_decay_schedule(
        0.0, args.lr, args.warmup_epochs * steps_per_epoch,
        total_steps, args.lr * args.min_lr_frac)
    optim = (optax.adamw(sched, weight_decay=args.weight_decay)
             if args.weight_decay > 0 else optax.adam(sched))
    ...
    for ep in range(1, args.epochs + 1):
        ...
        if ep % 10 == 0 or ep == 1:
            print(f"  ep {ep:4d} train={float(tl):.6f} val={vl:.6f} "
    ...
    if args.save_weights:
        ...
        np.savez(OUT_DIR / f"{args.name}_weights.npz",
                 weights_bytes=np.frombuffer(buf.getvalue(), dtype=np.uint8),
                 in_mean=in_mean, in_std=in_std, out_mean=out_mean,
                 config=json.dumps(vars(args)))

    report(args, band_names, params_raw[te], flux_test_true, mag_test_true,
           pred_flux, n_params, train_s)
    return 0


def report(args, band_names, params_te, flux_true, mag_true, pred_flux,
           n_params, train_s):
    import numpy as np

    np.savez_compressed(OUT_DIR / f"{args.name}_preds.npz",
                        pred_flux=pred_flux.astype(np.float32))
    ...
    res = {
        ...
        "per_band": {b: {"bias": float(bias[i]), "scatter": float(scatter[i]),
                         "p95": float(p95[i])} for i, b in enumerate(band_names)},
    }
    out = OUT_DIR / f"{args.name}.json"
    with open(out, "w") as fh:
        json.dump(res, fh, indent=2)
    print(f"\n==== {args.name} ====")
    for k in ["med_scatter", "med_abs_bias", "med_p95", "rmse", "trans_rms",
              "bright_flux_err", "det_scatter", "leak_p95_nJy", "leak_max_nJy",
              "worst_band_scatter", "n_params", "train_seconds"]:
        print(f"  {k:18s}: {res[k]}")
    print(f"saved -> {out}")
