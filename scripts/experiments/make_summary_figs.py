...
def load(name):
    """Load one experiment's metrics JSON by trial name."""
...
new = [
    load("S_win_100k")["det_scatter"],
    load("S_win_300k")["det_scatter"],
    load("A3_mass_sfr_f16")["det_scatter"],
]
base = [
    load("S_base_100k")["det_scatter"],
    load("S_base_300k")["det_scatter"],
...
ax.annotate(
    "production checkpoint (1M rows, full budget)",
    (1.05e5, ref * 1.05),
    fontsize=8.5,
    color="#3f6f5c",
)
...
for ax, vals, title, color in [
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [lab.replace("\n", " ") for lab in labels], fontsize=7.5, rotation=28, ha="right"
    )
