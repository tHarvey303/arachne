...
obs_flux = (true_flux + flux_err * np.array(jax.random.normal(nkey, true_flux.shape))).astype(np.float32)

print(f"n_bands={len(band_idx)}  n_params={len(SPS_PARAM_NAMES)}")

nss_init_fn, nss_step_fn = build_nss_fns(
    emu, band_idx, log_prior_fn, log_like_fn,
    num_inner_steps=24, num_delete=50,
)

t0 = time.perf_counter()
samples, logz, logz_err, ess, n_steps, n_dead, elapsed = run_nss_galaxy(
    rng, obs_flux, flux_err, nss_init_fn, nss_step_fn,
    num_live=500, termination=-3.0, n_samples_out=500, verbose=True,
)
