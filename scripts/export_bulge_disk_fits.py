...
def load_merged(path: Path) -> dict:
    """Load the merged NSS worker output HDF5 into a dict of arrays."""
...
def make_component_table(data: dict, component: str) -> Table:
    """Build the per-galaxy summary table for one component."""
    gid = data["galaxy_id"]
    z = data["z_fixed"]
    ...
    t["redshift"] = Column(z.astype("f4"), description="Fixed spectroscopic redshift (zfinal)")
    t["component"] = Column(
        np.full(N, component, dtype="U5"), description="Component: bulge or disk"
    )
    ...
    t["nss_logZ_err"] = Column(
        data["logZ_err"].astype("f4"), description="NSS log evidence MC uncertainty"
    )
...
def make_combined_table(B: dict, D: dict) -> Table:
    ...
    t["log_M_bulge"] = Column(
        np.percentile(Bsamps[ib, :, i_mass], 50, axis=1).astype("f4"),
        description="Bulge log M*/Msun median",
    )
    t["log_M_bulge_err_lo"] = Column(
        (
            np.percentile(Bsamps[ib, :, i_mass], 50, axis=1)
            - np.percentile(Bsamps[ib, :, i_mass], 16, axis=1)
        ).astype("f4")
    )
    t["log_M_bulge_err_hi"] = Column(
        (
            np.percentile(Bsamps[ib, :, i_mass], 84, axis=1)
            - np.percentile(Bsamps[ib, :, i_mass], 50, axis=1)
        ).astype("f4")
    )
    ...
    t["log_M_disk_err_lo"] = Column(
        (
            np.percentile(Dsamps[id_, :, i_mass], 50, axis=1)
            - np.percentile(Dsamps[id_, :, i_mass], 16, axis=1)
        ).astype("f4")
    )
    t["log_M_disk_err_hi"] = Column(
        (
            np.percentile(Dsamps[id_, :, i_mass], 84, axis=1)
            - np.percentile(Dsamps[id_, :, i_mass], 50, axis=1)
        ).astype("f4")
    )
...
def write_fits(table: Table, path: Path, description: str) -> None:
    """Write ``table`` to ``path`` as a FITS binary table with a description header."""
...
def main() -> None:
    """Export merged bulge/disk NSS results to FITS tables."""
    ...
    for name, tab in [("Bulge", Btab), ("Disk", Dtab)]:
        print(f"\n{name} summary:")
        print(
            f"  log_mass: {tab['log_mass'].mean():.2f} ± {tab['log_mass'].std():.2f}  "
